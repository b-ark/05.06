...
def get_phone_number(blank_string):
...
def get_str_from_user(description, blank_string=False):
...
def search_info(temp_str, temp_key):
...
def search_info_extended(temp_str1, temp_str2, temp_key1, temp_key2):
...
def search():
    ...
    чтобы у пользователя было больше вариантов найти нужную информацию, а не выбирать нужный"""
...
def update_contact(number, temp_key):
